...
main()
{
  ...
  IFS=' ' read -r -a plugins <<< $(get_tmux_option "@dracula-plugins" "battery hostname network cwd git cpu-usage")
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if case $plugin in custom:*) true;; *) false;; esac; then
      ...
      IFS=' ' read -r -a colors  <<< $(get_tmux_option "@dracula-cwd-colors" "red dark_gray")
      tmux set-option -g status-right-length 250
